...
def exit_ok(msg: str, /) -> None:
    log(Style.FG_GREEN + msg + Style.RESET)
    sys.exit()
...
def step(msg: str, /) -> Callable:
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            log(Style.BOLD + msg + Style.RESET)
            push_indent()
...
def validate_arguments() -> None:
    ...
    if g_args.uninstall and g_args.build_script is not None:
        exit_error(
            "The '--uninstall' flag cannot be used with the '--build-script' flag. The latter is used to run a build script after the setup. '--uninstall' is used to uninstall dependencies once the project is meant to be removed."
        )
